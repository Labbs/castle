...
export default {
  ...
  methods: {
    async onSubmit(repository) {
      const response = await fetchWrapper.post(`${import.meta.env.VITE_API_URL}/repository/${this.$route.params.id}`, repository);
      if (response.message === 'Repository edited') {
        this.$router.push('/repository');
      }
    },
    async getRepository() {
      const response = await fetchWrapper.post(`${import.meta.env.VITE_API_URL}/repository/update/${this.$route.params.id}`);
      ...
      <div class="page-header d-print-none">
        <div class="container-xl">
          <div class="row g-2 align-items-center">
            <div class="col">
              <h2 class="page-title">Update repository</h2>
            </div>
          </div>
        </div>
      </div>
